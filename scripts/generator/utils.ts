...
import type { AddressInfo, Addresses } from '../configs/types';
...
export function addressInfoToSolidityLibraryConstant(chainId: number, key: string, entry: AddressInfo) {
	if (typeof entry === 'object') {
		if (entry.type === 'uint256' || entry.type === 'uint8') {
			return `${entry.type} internal constant ${key} = ${entry.value};\n`;
		}
		const blockExplorerLinkComment = getExplorerLink(entry.chainId || chainId, entry.value.toString() as Hex);
		if (entry.type === 'address')
			return `// ${blockExplorerLinkComment}\naddress internal constant ${key} = ${getAddress(
				entry.value.toString() as Hex,
			)};\n`;
		return `// ${blockExplorerLinkComment}\n${entry.type} internal constant ${key} = ${
			entry.type
		}(${getAddress(entry.value.toString() as Hex)});\n`;
	}
...
export function generateSolidityConstants({
	...
	return Object.keys(addresses).map((key) => addressInfoToSolidityLibraryConstant(chainId, key, addresses[key]));
}
