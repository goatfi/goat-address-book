...
const main = async () => {
    ...
    for (const chainVaults of vaults) {
        ...
        if (invalidOwners === 0) printSuccess(`All Vaults and Strategies on ${chainVaults.chainId} have the Timelock as owner`);
        else pass = false;
    }
    ...
    for (const chainMultistrategies of multistrategies) {
        ...
        for (const multistrategy of chainMultistrategies.multistrategies) {
            let isValidOwner = await checkOwnership(multistrategy.address, desiredOwner, viemClient);
            const isValidManager = await checkManager(multistrategy.address, desiredManager, viemClient);
            if (!isValidOwner) {
                invalidOwners++;
            ...
            for (const adapter of multistrategyAdapters) {
                isValidOwner = await checkOwnership(adapter, desiredOwner, viemClient);
                if (!isValidOwner) {
                    invalidOwners++;
                    printOwnerError(adapter, chainMultistrategies.chainId);
                }
            }
        }
        if (invalidOwners === 0) printSuccess(`All Multistrategies and Adapters on ${chainMultistrategies.chainId} have the Timelock as owner`);
        else pass = false;
    }
...
const getDesiredManager = (chain: ChainId): Hex => {
    switch (chain) {
        case 42161:
            return ProtocolArbitrum.TREASURY;
        case 8453:
